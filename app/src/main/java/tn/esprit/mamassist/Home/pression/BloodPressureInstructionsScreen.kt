...
@Composable
fun BloodPressureInstructionsScreen(navController: NavController) {
    Scaffold(
        topBar = {
            TopAppBar(
                title = {
                ...
                navigationIcon = {
                    androidx.compose.material.IconButton(onClick = { navController.navigate("MainInterface") }) {
                ...
                colors = TopAppBarDefaults.smallTopAppBarColors(containerColor = Color(0xFFF8F4F1))
            )
        },
        content = { paddingValues ->
            Column(
                ...
                Column(
                    ...
                    InstructionItem(
                        number = 1,
                        text = "Asseyez-vous correctement, les deux pieds à plat sur le sol."
                    )
                    InstructionItem(
                        number = 2,
                        text = "Posez votre bras avec le brassard sur une table."
                    )
                    InstructionItem(number = 3, text = "Détendez-vous quelques minutes.")
                    InstructionItem(
                        number = 4,
                        text = "Ne parlez pas pendant la mesure de tension."
                    )
                }
...
fun PreviewBloodPressureInstructionsScreen() {
    BloodPressureInstructionsScreen(navController = rememberNavController())
}